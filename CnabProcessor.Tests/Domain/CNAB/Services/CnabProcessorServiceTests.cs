...
public class CnabProcessorServiceTests
{
    ...
    public async Task GetStoreSummariesAsync_ShouldReturnStoreSummaries()
    {
        ...
        {
            new Store { Id = 1, Owner = "JOÃO MACEDO", Name = "BAR DO JOÃO", Transactions = new List<Transaction>() }
        };
        
        mockUnitOfWork.Setup(u => u.Stores).Returns(mockStoreRepository.Object);
        mockStoreRepository.Setup(s => s.GetStoresWithTransactionsAsync())
                          .ReturnsAsync(stores);
        
        var service = new CnabProcessorService(mockUnitOfWork.Object, mockParserService.Object);

        // Act
        var result = await service.GetStoreSummariesAsync();

        // Assert
        result.Should().HaveCount(1);
        result[0].Owner.Should().Be("JOÃO MACEDO");
        result[0].Name.Should().Be("BAR DO JOÃO");
    }
}
